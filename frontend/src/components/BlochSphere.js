...
function BlochVisualization() {
    const [aliceBits, setAliceBits] = useState([0, 1, 0, 1, 1, 0, 0, 1, 0, 1, 1, 0, 0, 1, 0, 1]);
    const [aliceBases, setAliceBases] = useState(['Z', 'X', 'Z', 'X', 'Z', 'X', 'Z', 'X', 'Z', 'X', 'Z', 'X', 'Z', 'X', 'Z', 'X']);
    const [imageUrl, setImageUrl] = useState(null);

    const fetchBlochImage = async () => {
        try {
            const response = await fetch('http://localhost:8000/bloch_visualization', {
                method: 'POST',
                body: JSON.stringify({ alice_bits: aliceBits, alice_bases: aliceBases }),
                headers: { 'Content-Type': 'application/json' }
            });
